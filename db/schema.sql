...
\restrict MtZnW73Wph8n7ZtH5cdcOnROZkknS203iQ3wjV8bowpEO0gZlRN3sYtl9hafUhr
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- Detect soft-delete (works via JSONB — no crash if column doesn't exist)
    ...
    -- Compute changed fields (exclude noise)
    v_changed := (
      SELECT jsonb_object_agg(n.k, n.v)
      FROM jsonb_each(v_new_data) AS n(k, v)
      LEFT JOIN jsonb_each(v_old_data) AS o(k, v) ON n.k = o.k
      WHERE n.v IS DISTINCT FROM o.v
        AND n.k NOT IN ('modified_at', 'version')
    );
...
\unrestrict MtZnW73Wph8n7ZtH5cdcOnROZkknS203iQ3wjV8bowpEO0gZlRN3sYtl9hafUhr
